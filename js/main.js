(function ($) {
  // =========================
  // DOM Cache (restored + tiny additions)
  // =========================
  ...
  // Optional caption element. If you add <p id="projectMainImageDescription"></p> next to the main image,
  // the code will use it. Safe when not present.
  const $projectMainImageDescription = $('#projectMainImageDescription');
  ...
  // Read images from detail with backward compatibility and include optional "label"
  function normaliseImages(detail) {
    const images = Array.isArray(detail.images) ? detail.images : [];
    const fallbackAlt = detail.title ? detail.title + ' preview image' : 'Project preview image';
    if (!images.length) {
      return [{ src: 'img/work-1.jpg', alt: fallbackAlt, description: '', label: '' }];
    }
    return images.map(img => {
      if (!img) return null;
      if (typeof img === 'string') return { src: img, alt: fallbackAlt, description: '', label: '' };
  ...
  function setMainImage(image, fallbackDescription) {
    const hasImage = image && image.src;
    const altText = (image && image.alt) || 'Project preview image';
    const description = (image && image.description) || fallbackDescription || '';

    if (!hasImage) {
      $projectMainImage.attr('src', '').attr('alt', '');
      if ($projectMainImageDescription && $projectMainImageDescription.length) {
        $projectMainImageDescription.text('').prop('hidden', true);
      }
      return;
    }

    $projectMainImage.attr('src', image.src).attr('alt', altText);
    const labelBox = document.getElementById("projectMainImageLabel");
    if (labelBox) {
      if (image.label) {
        labelBox.textContent = image.label;
    ...
    if ($projectMainImageDescription && $projectMainImageDescription.length) {
      if (description) {
        $projectMainImageDescription.text(description).prop('hidden', false);
      } else {
        $projectMainImageDescription.text('').prop('hidden', true);
      }
    }
  ...
  // Build the thumbnail strip. If an image has "label", show it under the thumb.
  function renderImages(detail, fallbackDescription) {
    ...
    setMainImage(primary || null, fallback);
    ...
    images.forEach(image => {
      ...
      const $btn = $('<button type="button" class="project-thumbnail"></button>')
        .attr('aria-label', image.label || image.alt || 'Preview image');

      // Image frame
      const $frame = $('<div class="project-thumbnail__frame"></div>');
      $frame.append($('<img/>', { src: image.src, alt: image.alt }));

      // Optional label
      const labelText = image.label || '';
      const $label = labelText
        ? $('<div class="project-thumbnail__label"></div>').text(labelText).attr('title', labelText)
        : null;

      if (isFirst) {
        $btn.addClass('is-active');
        isFirst = false;
      } else if (primary && image.src === primary.src) {
        $btn.addClass('is-active');
      }
  ...
  function populateProjectModal(id) {
    ...
    if (!detail) return;
    const card =
      projectCards.find(item => item.id === id) ||
      awardsCards.find(item => item.id === id) ||
      activityDetails[id];
    const fallbackImageDescription =
      card && typeof card.imageDescription === 'string'
        ? card.imageDescription.trim()
        : '';
    ...
    renderImages(detail, fallbackImageDescription);
  ...
  function openCertificateModal(context) {
    ...
    $certificateModalDescription.html(description);
    ...
    const $modalRoot = $certificateModal;
